(() => {
  ...
  function card(r, n) {
    ...
    return `<article class="banner-row" id="banner-row-${n}" tabindex="-1" data-banner-id="${escape(r.id)}" data-record-key="${escape(r.key)}"><div class="old-cell"><div class="record-top"><span class="row-number">${String(n).padStart(3,'0')}</span>${terminals}${variantHint}</div><div class="id-line"><span class="id-label">ID</span><span class="id-value">${escape(r.id)}</span><button type="button" class="icon-button" data-copy="${escape(r.id)}" aria-label="复制 Banner ID ${escape(r.id)}">${icon('copy')}</button></div><h2 class="record-title">${escape(r.title)}</h2>${preview}<div class="brief-meta" data-meta-key="${escape(r.key)}">${icon('pin')}<span class="brief-meta-text" id="meta-${escape(r.key)}">${escape(r.fields['可用城市'] || '未设置城市')}<span class="meta-separator">/</span>${escape(r.fields['运营公司'] || '未设置运营公司')}</span><button type="button" class="meta-toggle" data-meta-toggle aria-expanded="false" aria-controls="meta-${escape(r.key)}" hidden>展开</button></div><details class="details" data-details-key="${escape(r.id)}" ${openedDetails.has(r.id)?'open':''}><summary>${icon('arrow')}配置信息<span class="source-row">Excel 第 ${r.sourceRow} 行</span></summary><dl>${metadata(r)}</dl></details></div><div class="new-cell"><div class="new-label"><span>${next?.src?'已适配 8.0':'新版预留'}</span><span class="tiny-id">${escape(r.terminal)}</span></div>${newer}</div></article>`;
  }
  ...
  function setPreview(index) {
    ...
    $('preview-subtitle').textContent=`${previewVersion} · ${r.terminal} · ID ${r.id} · ${image.width} × ${image.height} px`;
